...
def track(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if method_name == '_validate_speaker_name':
            pp(args)
            pp(kwargs)
            owner=args[0]
            recipient, messages, sender, config, attempts_left, attempt, agents=args[1:]
            message=messages[-1]['content']
            params= f'{owner.name}: {sender.name}->{recipient.name}, "{message[:30]}"'
            #e()
    
        
        if method_name == 'agent_by_name':
            agent_name=args[1]
            owner=args[0]
            params= f'{owner.name}: {agent_name}'
            #e()

        if method_name == '_process_speaker_selection_result':
            #pp(args)
            #pp(kwargs)
            owner=args[0]
            result, last_speaker, agents = args[1:] 
            chat_history=result.chat_history[-1]['content']
            params= f'{owner.name}: last_speaker:{last_speaker.name}, {chat_history[:30]}'
            #e()
        
        if method_name == '_prepare_and_select_agents':
            owner=args[0]   
            #params= 
            params= f'{owner.name}: {args[1].name}'

        if method_name == '_auto_select_speaker': 
            pp(args)

            pp(kwargs)
            _,last_speaker, selector, messages, agents=args
            message=messages[0]['content']
            owner=args[0]
            params= f'{owner.name}: last_speaker: {last_speaker.name}, selector: {selector.name}, "{message[:30]}"'
            #e()       
        if method_name == 'select_speaker':

            last_speaker=args[1]
            owner=args[0]
            #params= last_speaker.name
            params= f'{owner.name}: last_speaker: {last_speaker.name}'
...
class GroupChat:
    ...
    def select_speaker(self, last_speaker: Agent, selector: ConversableAgent) -> Agent:
        ...
        print(self.__class__.__name__,"Selecting speaker")
        ...
        print("Selecting speaker automatically", last_speaker, last_speaker.name)
        #agents =[agent.name for agent in self.agents if agent!=last_speaker]
    ...
    def _auto_select_speaker(
        ...
        symsg= self.select_speaker_msg(agents)
        from pprint import pprint as pp
        print("symsg")
        ...
        print("select_auto_speaker: result",result)
        #e(123)
...
class GroupChatManager(ConversableAgent):
    ...
    def run_chat(
        ...
        for i in range(groupchat.max_round):
            ...
            for agent in groupchat.agents:
                if agent != speaker:
                    print(f"GropuChatManger:group chat [{groupchat.max_round}]run_chat: broadcast message to {agent.name}")
