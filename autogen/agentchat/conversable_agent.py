...
def track(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if method_name == '_print_received_message':
            ...
            message=args[1]
            sender=args[2]
            params= f'{owner.name}: {sender.name}, "{message[:30]}"'
            #e()
    

        if method_name == 'generate_function_call_reply':
            #pp(args)
            #pp(kwargs)
            owner=args[0]
            messages=kwargs['messages']
            sender=kwargs['sender']
            params= f'{owner.name}: {sender.name}, "{messages[0]["content"][:30]}"'
            #e()

        if method_name == '_summarize_chat':
            #pp(args)
            #pp(kwargs)
            owner=args[0]   
            summary_method=args[1] 
            recipient=args[3]
            params=f'{owner.name}: {summary_method}, {recipient.name}'
            #e()

        if method_name == 'generate_tool_calls_reply':
            #pp(args)
            #pp(kwargs)  
            owner=args[0]
            messages=kwargs['messages']
        ...
        if method_name == 'process_all_messages_before_reply':
          
            owner=args[0]
            messages=args[1]
            params=f'{owner.name}: "{messages[0]["content"][:30]}"'
            
        if method_name == 'check_termination_and_human_reply':
            pp(args)
            pp(kwargs)
            sender=kwargs['sender'] 
            messages=kwargs['messages']
            ...
            params=f'{owner.name}<-{sender.name}, "{messages[0]["content"][:30]}"'
            #e()
        if method_name == '_generate_oai_reply_from_client':
            #pp(args)
            #pp(kwargs)
            owner=args[0]
            llm_client=args[1]
            messages=args[2]
            params= f'{owner.name}: {llm_client.__class__.__name__}, "{messages[0]["content"][:30]}"'
            #e()
        if method_name == 'process_last_received_message':
            message=args[1]
            pp(args)
            owner=args[0]
            if type(message) == list:
                message=message[0]['content']
            params= f'{owner.name}: "{message[:30]}"'
        if method_name == '_process_received_message':
            pp(args)
            pp(kwargs)
            message, sender=args[1], args[2]
            owner=args[0]
            if type(message) == dict:
                message=message['content']
            params= f'{owner.name}: sender: {sender.name}, "{message[:30]}"'
        if method_name == '_process_message_before_send':
            pp(args)
            pp(kwargs)
            message, recipient=args[1], args[2]
            owner=args[0]
            if type(message) == dict:
                message=message['content']
            params= f'{owner.name}: recipient: {recipient.name}, "{message[:30]}"'
           # e()

        if method_name == 'generate_init_message':
            pp(args)
            pp(kwargs)
            message = args[1]
            owner=args[0]
            ...
            params= f'{owner.name}: "{message[:30]}"'
            #e()
        if method_name == 'register_reply':
            trigger=args[1]
            owner=args[0]
            if type(trigger ) == list:
                params= args[1][0].__name__
            else:
                params= args[1].__name__
            #pp(args)
            if len(args) > 2:
                fname= args[2].__name__
                params= f'{params},{fname}'
                print(params)
                if fname=='wrapper':
                    pp(params)
                    #e()
            params= f'{owner.name}: {params}'
        if method_name == '_prepare_chat':
            agent=args[1]
            owner=args[0]
            params= f'{owner.name}: {agent.name}'
        if method_name == '_append_oai_message':
            message=args[1]
            #print(444, message) 
            owner=args[0]
            if type(message) == dict:
        ...
        if method_name == '_match_trigger':
            ...
            if isinstance(trigger, list):
                pp(trigger)
                params=trigger[0].__name__
                pass

            else:

                print(trigger.__name__)
                params=trigger.__name__
                #e()
            params= f'{owner.name}: {params}'   
        if method_name == 'generate_oai_reply':
            #messages    = args[1]
            params=''
            owner=args[0]
            if len(args) ==1:
                params=f'{owner.name}, No args'
            else:
                messages    = args[1]
                #param='No args'
                pp(messages)
        ...
        if method_name == 'send':
            ...
            if type(message) == dict:
                message=message['content']
            params= f'{owner.name}->{recipient.name}, "{message[:30]}"'


        if method_name == 'generate_reply':
        ...
        branch['calling'][apc.call_id]={'name': f'{method}({params})','depth':apc.depth,'calling':{},'caller':apc.depth-1}
...
class ConversableAgent(LLMAgent):
    ...
    def send(
        ...
        """
        print ('send start: request_reply', request_reply, message, self.name,'->>',recipient.name)
        ...
        if valid:
            print('send:',message, self, request_reply, silent)
    ...
    def receive(
        ...
        if request_reply is False or request_reply is None and self.reply_at_receive[sender] is False:
            ...
            print(request_reply, self.reply_at_receive[sender])
            return
        ...
        **kwargs,
        ...
        if isinstance(max_turns, int):
            ...
            for _ in range(max_turns):
                print('CA initiate_chat: max_turns:', max_turns,_)
                if _ == 0:
                    ...
                    #pp(self.chat_messages[recipient])
        ...
        print(f'exiting initiate_chat: ConversableAgent:{self.name}->initiate_chat: chat_result:', chat_result)
        return chat_result
    ...
    def _generate_oai_reply_from_client(self, llm_client, messages, cache) -> Union[str, Dict, None]:
        ...
        print ('in _generate_oai_reply_from_client: messages:',all_messages)
        response = llm_client.create(
            context=messages[-1].pop("context", None), messages=all_messages, cache=cache, agent=self
        )
        print ('in _generate_oai_reply_from_client: response:',response)    
        ...
        **kwargs: Any,
        ...
        for reply_func_tuple in self._reply_func_list:
            ...
            if self._match_trigger(reply_func_tuple["trigger"], sender):
                print('generate_reply: reply_func:',reply_func)
                print ('generate_reply: trigger:', type(reply_func_tuple['trigger']))
                #for t in reply_func_tuple['trigger']:
                #    print('\tgenerate_reply: trigger:',type(t))
                print('generate_reply: config:',reply_func_tuple['config'])
                #pp(reply_func_tuple["config"])
                #pp(self.llm_config)
                print(f"CA:generate_reply:    SENDER.NAME {sender.name}")
                print(f"CA:generate_reply:    MESSAGES {messages}")                
                final, reply = reply_func(self, messages=messages, sender=sender, config=reply_func_tuple["config"])
                #e(777)
                print('generate_reply: post reply_func:', reply_func)
                print('generate_reply: post reply:',final, reply)
                if logging_enabled():
                    log_event(
                        self,
                        "reply_func_executed",
